...
static void ST_overlayDrawer(void)
{
	...
	if ((gametype == GT_COOP || gametype == GT_RACE) && !(hu_showscores && (netgame || multiplayer)))
	{
		if ((stplyr->powers[pw_super]) || (stplyr->pflags & PF_NIGHTSMODE))
		{
			...
			{
				V_DrawSmallScaledPatch(SCX(hudinfo[HUD_LIVESPIC].x), SCY(hudinfo[HUD_LIVESPIC].y - 0),
			...
			else
			{
				...
				V_DrawSmallMappedPatch(SCX(hudinfo[HUD_LIVESPIC].x), SCY(hudinfo[HUD_LIVESPIC].y - 0),
...
void ST_MovieInfoDrawer(void)
{
	...
	V_DrawString(x, y,
		movietype_color|V_TRANSLUCENT|V_SNAPTOLEFT|V_SNAPTOBOTTOM,
		movietype
	);
	...
	V_DrawString(strlen(movietype) * 10 + x, y,
		V_ALLOWLOWERCASE|V_TRANSLUCENT|V_SNAPTOLEFT|V_SNAPTOBOTTOM,
		va(
